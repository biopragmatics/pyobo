...
def get_ontology_annotations(obo_ontology: Obo) -> Iterable[f.Annotation]:
    ...
    for predicate, value in obo_ontology._iterate_property_pairs():
        yield f.Annotation(predicate, _convert_literal_or_reference(value))
    if obo_ontology.data_version:
        literal = OBOLiteral.string(obo_ontology.data_version)
        yield f.Annotation(pv.version_info, _oboliteral_to_literal(literal))
    if obo_ontology.auto_generated_by is not None:
        literal = OBOLiteral.string(obo_ontology.auto_generated_by)
        yield f.Annotation(pv.obo_autogenerated_by, _oboliteral_to_literal(literal))
