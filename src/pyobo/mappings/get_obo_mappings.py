...
DB_PATH = os.path.join(HERE, 'meta_mappings.json')
...
XREF_PREFIX_BLACKLIST = set(DB['blacklists']['prefix'])
XREF_SUFFIX_BLACKLIST = set(DB['blacklists']['suffix'])
#: Xrefs matching these will be ignored
XREF_BLACKLIST = set(DB['blacklists']['full'])

PREFIX_REMAP = DB['remappings']['prefix']

ALLOWED_UNNORM = set(DB['database'])
...
def get_namespace_synonyms() -> Mapping[str, str]:
    ...
    synonym_to_key = {}
    for key, values in DB['database'].items():
        synonym_to_key[key] = key
        synonym_to_key[key.upper()] = key

        for synonym in values.get('synonyms', []):
            synonym_to_key[synonym] = key
            synonym_to_key[synonym.lower()] = key
            synonym_to_key[synonym.upper()] = key
            synonym_to_key[synonym.casefold()] = key

            for x, y in [('_', ' '), (' ', '_'), (' ', '')]:
                synonym_to_key[synonym.replace(x, y)] = key
                synonym_to_key[synonym.lower().replace(x, y)] = key
                synonym_to_key[synonym.upper().replace(x, y)] = key
                synonym_to_key[synonym.casefold().replace(x, y)] = key
...
def iterate_xrefs_from_graph(graph: nx.Graph, obo_key: str):
    for node, data in graph.nodes(data=True):
        ...
        for xref in data.get('xref', []):
            ...
            for k, v in PREFIX_REMAP.items():
                if xref.startswith(k):
                    xref = v + xref[len(k):]
...
def main(directory):
    ...
    export_path = os.path.join(directory, f'xrefs.tsv')
