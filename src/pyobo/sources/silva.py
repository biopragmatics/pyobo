...
def iter_terms_silva(version: str, force: bool = False) -> Iterable[Term]:
    ...
    for ena_embl_id, species_silva_taxon_id in tqdm(
        ...
        term_ = silva_taxon_id_to_term.get(species_silva_taxon_id)
        if term_ is None:
            logger.warning(
            ...
            if ref not in term_.properties[RELATION_NEEDS_NEW_NAME.reference]:
                term_.annotate_object(RELATION_NEEDS_NEW_NAME, ref)
