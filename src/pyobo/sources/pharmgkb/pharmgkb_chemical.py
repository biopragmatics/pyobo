...
from pyobo.sources.pharmgkb.utils import download_pharmgkb_tsv
...
def iter_terms(force: bool = False) -> Iterable[Term]:
    ...
    for _, row in df.iterrows():
        ...
        for xref_curie in _split(row, "Cross-references"):
            try:
                reference = Reference.from_curie(xref_curie)
            except ValueError:
                pass
        ...
        for trade_name in _split(row, "Trade names"):
...
if __name__ == "__main__":
    PharmGKBChemicalGetter().write_default(force=True, write_obo=True)
